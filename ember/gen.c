...
/*
 * dis instruction assembly: instruction list management,
 * operand addressing, temporaries, branch folding, and final
 * pc/descriptor resolution.  ported from limbo/gen.c; the
 * addressing mode tables and pc resolution must match what
 * libinterp/load.c expects to decode.
 */
...
genrawop(Src *src, int op, Node *s, Node *m, Node *d)
{
	...
	if(in->sm != Anone || in->mm != Anone || in->dm != Anone)
		fatal("bogus mkinst in genrawop: %I", in);
	if(s != nil){
		in->s = genaddr(s);
	...
	if(m != nil){
		...
		if(in->mm == Ampind || in->mm == Afpind)
			fatal("illegal addressing mode in register");
	}
	if(d != nil){
		in->d = genaddr(d);
...
genop(Src *src, int op, Node *s, Node *m, Node *d)
{
	...
	if(iop == 0)
		fatal("can't deal with op %s on %T in genop", opname[op], d->ty);
	in = mkinst();
	in->op = iop;
	in->src = *src;
	if(s != nil){
...
genbra(Src *src, int op, Node *s, Node *m)
{
	...
	if(iop == 0)
		fatal("can't deal with op %s on %T in genbra", opname[op], t);
	in = mkinst();
	in->op = iop;
	in->src = *src;
	if(s != nil){
...
genchan(Src *src, Node *sz, Type *mt, Node *d)
{
	...
	op = chantab[mt->kind];
	if(op == 0)
		fatal("can't deal with type %T in genchan", mt);
...
genmove(Src *src, int how, Type *mt, Node *s, Node *d)
{
	...
	if(op == 0)
		fatal("can't deal with how %d type %T in genmove", how, mt);
...
/*
 * follow all possible paths from in,
...
genaddr(Node *n)
{
	...
	case Rpc:
		...
		break;
	default:
		fatal("can't deal with addressable %d in genaddr", n->addable);
		break;
	}
	return a;
}
 ...
 */
static	Inst*	rewritesrcreg(Inst*, int, int, int);
static	Inst*	rewritedestreg(Inst*, int, int);
...
resolvepcs(Inst *inst)
{
	...
	for(in = inst; in != nil; in = in->next){
		...
		case Aldt:
			...
			break;
		default:
			fatal("can't deal with %I's m mode", in);
			break;
		}
...
/*
 * fix up a big register constant used as a source
 */
static Inst*
