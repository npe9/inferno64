...
/*
 * lib9.h sets _XOPEN_SOURCE, which hides Darwin extensions in
 * <sys/mman.h>. Provide the values ourselves.
 */
#ifndef MAP_ANON
...
/* Register-offset loads (option=UXTX/LSL, amount in S bit / imm) */
#define LDRB_REG(Rt, Rn, Rm) \
	*code++ = (0x38606800 | ((Rm)<<16) | ((Rn)<<5) | (Rt))
#define LDR32_REG_LSL2(Rt, Rn, Rm) \
	*code++ = (0xB8607800 | ((Rm)<<16) | ((Rn)<<5) | (Rt))	/* LSL #2 */
...
/*
 * Conditional branch to a Dis PC. Always two slots (short B.cond+NOP or
 * inverted B.cond + B) so pass0/pass1 stay aligned — same idea as bcondbra.
 */
static void
bconddis(int cond, int dispc)
{
	...
	off -= 1;
	if(off > 0x1FFFFFF || off < -0x2000000) {
		print("bconddis long-form overflow: off=%ld dispc=%d\n",
			off, dispc);
		urk("bconddis: branch too far");
	}
	B_IMM(off);
}
...
comp(Inst *i)
{
	...
	case IINDC: {
		...
		BCOND(GE, 0);
		if(sizeof(Rune) == 4)
			LDR32_REG_LSL2(RA3, RA1, RA2);
		else
			*code++ = (0x78607800 | (RA2<<16) | (RA1<<5) | RA3);
		done = code;
		B_IMM(0);
		PATCH_BCOND(ascii);
		LDRB_REG(RA3, RA1, RA2);
		PATCH_B(done);
		opwst(i, Stw, RA3);
		break;
	}
...
macmfra(void)
{
	/* X30 holds resume PC (set by commframe before bramac). */
	mem(Stw, O(REG, st), RREG, 30);
	mem(Stw, O(REG, s), RREG, RA3);
	mem(Stw, O(REG, d), RREG, RA0);
	mem(Stw, O(REG, FP), RREG, RFP);
	...
	BLR_REG(RTA);
	con((uvlong)&R, RREG);
	mem(Ldw, O(REG, st), RREG, 30);	/* restore resume PC */
	mem(Ldw, O(REG, FP), RREG, RFP);
	mem(Ldw, O(REG, MP), RREG, RMP);
	RET_X30();
}
