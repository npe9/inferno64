...
# GAP: no map scroll widgets / GrFloodFill terrain
...
guns, gunmasks: array of ref Image;
elevs: array of int;
...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	guns = array[3] of ref Image;
	gunmasks = array[3] of ref Image;
	for(i = 1; i <= 2; i++){
		guns[i] = d.open(sys->sprint("/icons/temple/bigguns_%d.bit", i));
		gunmasks[i] = d.open(sys->sprint("/icons/temple/bigguns_%d.mask", i));
	}
...
redraw()
{
	...
	if(gx >= -40 && gx < w + 40){
		...
		if(guns[2] != nil)
			draw3d->sprite2d(img, Point(o.x+gx, o.y+gy-5), guns[2], gunmasks[2],
				guns[2].r.dx(), guns[2].r.dy(), 0.0, 0);
		else
			img.fillellipse(Point(o.x+gx, o.y+gy), 8, 6, ink[0], Point(0, 0));
		if(guns[1] != nil){
			cx := bx+15.0*c;
			cy := by+15.0*s;
			draw3d->sprite2d(img, Point(o.x+int cx, o.y+int cy),
				guns[1], gunmasks[1], guns[1].r.dx(), guns[1].r.dy(),
				gunang*180.0/PI, 0);
		}else{
			ex := bx + 30.0*c;
			ey := by + 30.0*s;
