...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	if(env != nil){ env->clone();
	 env->setenv("wmman", "danby-ecology");
	 }
	sys->pctl(Sys->NEWPGRP, nil);
	wmclient->init();
	if(ctxt == nil) ctxt = wmclient->makedrawcontext();
	win = wmclient->window(ctxt, "Predator and prey", Wmclient->Appl);
...
redraw()
{
	...
	im.text(im.r.min.add((10,18)),fg,Point(0,0),font,sys->sprint("Predator and prey — t %.2f  prey %.4g  predators %.4g",t,y[0],y[1]));
	drawpar(im,0,"prey birth",birth,3.0);
	drawpar(im,1,"predation",predation,4.0);
	drawpar(im,2,"predator death",death,3.0);
