...
histt, hists, histi, histr, histv: array of real;
nh := 0;
...
redraw()
{
	...
	im.line((x0,map(histv[i-1],0.0,1.0,r.max.y,r.min.y)),(x1,map(histv[i],0.0,1.0,r.max.y,r.min.y)),0,0,1,grid,Point(0,0));
	}
	im.text(im.r.min.add((10,18)),fg,Point(0,0),font,sys->sprint("Vaccination threshold — t %.2f  S %.3f  I %.3f  R %.3f  V %.3f",t,y[0],y[1],y[2],y[3]));
