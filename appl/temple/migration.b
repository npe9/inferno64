...
include "env.m";
 env: Env;
Migration: module { init: fn(ctxt: ref Draw->Context, argv: list of string);
 };
win: ref Window;
font: ref Font;
bg, fg, grid, live, accent: ref Image;
y := array[] of {0.85, 0.12};
growth := 1.0;
migration := 0.18;
t := 0.0;
paused := 0;
...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	if(sys == nil || draw == nil || wmclient == nil || numerics == nil)
		raise "fail:Migration: missing module";
	if(env != nil){ env->clone();
	 env->setenv("wmman", "danby-migration");
	 }
	sys->pctl(Sys->NEWPGRP, nil);
	wmclient->init();
	if(ctxt == nil) ctxt = wmclient->makedrawcontext();
	win = wmclient->window(ctxt, "Patch migration", Wmclient->Appl);
...
{
	flow := migration*(q[1]-q[0]);
	d[0]=growth*q[0]*(1.0-q[0])+flow;
	d[1]=growth*q[1]*(1.0-q[1])-flow;
}
reset()
{
	y[0]=0.85;
...
redraw()
{
	...
	if(y[1] > y[0])
		direction = "B → A";
	im.text(im.r.min.add((10,18)),fg,Point(0,0),font,sys->sprint("Patch migration — t %.2f  A %.3f  B %.3f  flow %s %.3f",t,y[0],y[1],direction,migration*absr(y[0]-y[1])));
	drawpar(im,0,"growth",growth,3.0);
	drawpar(im,1,"migration",migration,1.0);
