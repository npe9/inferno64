...
# GAP: Sprite3ZB rotation — we draw optional sprite without Z-rotate
...
kleft := 0;
kright := 0;
thrust_until := 0;	# millisec linger (no key-up events)
have_tone := 0;
gy: int;
...
paint()
{
	...
	if(ship != nil && smask != nil){
		p0 := Point(rx - ship.r.dx()/2, ry - ship.r.dy()/2);
		buffer.draw(ship.r.addpt(p0), ship, smask, ship.r.min);
	}else{
