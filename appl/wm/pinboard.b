...
Bgcolor:  con "#C4C0B4";	# Soft Plan9 Paper desktop (matches wm Background)
Fgcolor:  con "#2A2A22";	# soft near-black icon labels
Selcolor: con "#3D6A9A";	# soft accent selection outline
...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	(off, data, nil, wc) := <-pinIO.write =>
		if(wc == nil)
			break;
		nread := len data;
		# Cast for Inferno variants where FileIO offset is big.
		ioff := int off;
		if(ioff == 0 && len residual > 0)
			residual = nil;
	...
	(off, nbytes, nil, rc) := <-pinIO.read =>
		...
		if(ioff == 0)
			readsnap = snapshot();
		data: array of byte;
		if(ioff < len readsnap){
			e := ioff + nbytes;
			if(e > len readsnap)
				e = len readsnap;
			data = readsnap[ioff:e];
		}
		alt {
		rc <-= (data, nil) => ;
		* => ;
