...
# the synthesis graph (adsr/mixer/delay/onepole/onezero/twopole/
# twozero/lfo/waveloop/fm/master/poly) is unchanged from the original.
# Two real adaptations: audioctl()/the audio device paths, to match
# this tree's own verified-working /dev/audio convention (see tone(2))
# rather than the original's #A/audioctl; and init()/play() both
# called sys->pctl() through the module-level sys variable BEFORE
# modinit() had loaded it - a latent bug invisible in whatever
# environment this was originally written against (where Sys may have
# come pre-bound into every process's module table), but a genuine
# "module not loaded" crash on the very first line in this build,
# where it doesn't. Fixed by loading first.
implement Sequencer;
...
# --- Session: a small command language wrapping one master Inst ---
...
newsession(): ref Session
{
	if(sys == nil)
		modinit();
	inst := Inst.mk(nil, master);
	return ref Session(inst, chan of string, 0);
}

cmdwords(s: string): array of string
{
	l := str->fields(s);
...
playrun(s: ref Session, file: string)
{
	play(file, s.playctl, s.inst);
	s.playing = 0;
}

Session.cmd(s: self ref Session, arg: string): string
{
	if(sys == nil)
		modinit();
	a := cmdwords(arg);
	...
	"noteon" =>
		if(len a != 3)
			return "usage: noteon voice midinote";
		voice := int a[1];
		midi := int a[2];
		if(voice < 0 || voice >= 4)
			return "noteon: voice must be 0..3";
		s.inst.ctl <-= (CVOICE, real voice);
		s.inst.ctl <-= (CKEYON, midi2pitch[midi%len midi2pitch]);
	"noteoff" =>
		if(len a != 2)
			return "usage: noteoff voice";
		voice := int a[1];
		if(voice < 0 || voice >= 4)
			return "noteoff: voice must be 0..3";
		s.inst.ctl <-= (CVOICE, real voice);
		s.inst.ctl <-= (CKEYOFF, 0.0);
	"play" =>
		if(len a != 2)
			return "usage: play file";
		if(s.playing)
			return "play: already playing - stop first";
		s.playing = 1;
		spawn playrun(s, a[1]);
	"stop" =>
		if(s.playing)
			s.playctl <-= "stop";
	* =>
		return "sequencer: unknown command " + a[0];
	}
	return nil;
}
...
{
	fd := sys->open("/dev/audioctl", Sys->OWRITE);
