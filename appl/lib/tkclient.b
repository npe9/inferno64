...
#sys->print("wmctl %s\n", req);
	...
	"source" =>
		if((err := titlebar->develop(dispath)) != nil)
			sys->fprint(sys->fildes(2), "tkclient: %s\n", err);
	"rect" =>
		r: Rect;
...
showman(ctxt: ref Draw->Context)
{
	...
	if(!manplumbready){
		if(plumbmsg->init(1, nil, 0) < 0){
			sys->fprint(sys->fildes(2), "tkclient: cannot connect to plumber: %r\n");
			return;
		}
		manplumbready = 1;
	}
	...
	if(msg.send() < 0)
		sys->fprint(sys->fildes(2), "tkclient: cannot plumb manual %s: %r\n", path);
}
