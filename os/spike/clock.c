...
{
	/* per-hart mtimecmp; we run on BOOT_HART */
	*(volatile uvlong*)(uintptr)(CLINT + ClintMtimecmp + 8*BOOT_HART) = when;
}
...
clockpoll(void)
{
	...
	ticksync();
	cmp = *(volatile uvlong*)(uintptr)(CLINT + ClintMtimecmp + 8*BOOT_HART);
	if(clintmtime() >= cmp)
		clockintr(nil);
}
