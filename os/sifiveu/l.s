...
#define	MEIE	2048		/* mie.MEIE — machine external (PLIC) */
#define	FSBITS	(3<<13)		/* mstatus.FS = Dirty — enable FPU */
#define	MRET	WORD $(0x30200073)
#define	BOOT_HART_ID	1	/* U54; hart0 E51 has no FPU */
...
/*
 * Entry for QEMU -M sifive_u -bios none -kernel isifiveu.
 * Park E51 (hart0); boot first U54 (hart1) with FPU.
 * Stack is SB-relative so RV64 lui sign-extension is avoided.
 */
TEXT	_main(SB), 1, $-8
	MOV	CSR(MHARTID), R10
	MOV	$BOOT_HART_ID, R11
	BNE	R10, R11, park
	MOV	$setSB(SB), R3
